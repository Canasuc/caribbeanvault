      ...
      {!rgpdConsented ? (
        ...
        <>
          <p className="text-sm text-gray-600 mb-6">
            {isMobileDevice ? t("steps.connect.description_mobile") : t("steps.connect.description_desktop")}
          </p>
...
useEffect(() => {
  setIsMobileDevice(/iPhone|iPad|Android/i.test(navigator.userAgent));
}, []);

  const suggestedStablecoin: StablecoinChoice = EUR_LOCALES.includes(locale) ? "geur" : "rlusd";

  // En mode reconnect : on saute directement à l'étape connect
  const initialStep: OnboardingStep = mode === "reconnect" ? "connect" : "questionnaire";
  const initialSteps: OnboardingStep[] = mode === "reconnect" ? ["connect", "complete"] : [];
  ...
  const [state, setState] = useState<OnboardingState>({
    ...
    rgpdConsented: true, // En mode reconnect, le RGPD a déjà été accepté
  ...
  async function initiateWalletConnect() {
    try {
      ...
      if (isMobileDevice && data.mobileUrl) {
  ...
  async function saveToSupabase() {
    ...
    if (mode === "reconnect") {
      // Mode reconnect — juste mettre à jour connected_at et l'adresse confirmée
      await supabase.from("investisseurs").update({
        connected_at: new Date().toISOString(),
        xrpl_address: state.xrplAddress || undefined,
      }).eq("id", investorId);
    } else {
      // Mode onboarding complet
      await supabase.from("investisseurs").update({
